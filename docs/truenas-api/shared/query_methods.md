...
title: Query Methods
kind: shared
source_html: query_methods.html
---
...
When the path to the key contains an array, an array index may be manually specified. When passed to the `privilege.query` endpoint, the following query filters will return entries where the first element of the local groups array has a name of “myuser”.
...
To get the ‘last’ or most recent entry on a query, first get the `count` then use the same query filter and an `offset` value of `count - 1`.
### Select
